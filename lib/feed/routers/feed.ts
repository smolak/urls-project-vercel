...
const getUserFeeds = protectedProcedure.query(async ({ ctx: { logger, requestId, session, prisma } }) => {
  ...
  try {
    ...
    throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch user's feed list.", cause: error });
  }
});
